...
void DownloadTask::loadVersionInfo()
{
	...
	netJob->addNetAction(m_newVersionFileListDownload = Download::makeByteArray(newIndexUrl, &newVersionFileListData));
	...
	if (!m_status.currentRepoUrl.isEmpty())
	{
		...
		netJob->addNetAction(m_currentVersionFileListDownload = Download::makeByteArray(cIndexUrl, &currentVersionFileListData));
